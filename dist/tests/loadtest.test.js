...
function makeResult(summaryOverrides = {}) {
    return {
        run_id: 'test-run-001',
        snapshots: [],
...
describe('LoadTestExportService', () => {
    ...
    it('throws for unknown framework', () => {
        assert.throws(() => svc.getExporter('unknown'), /Unknown load test framework/);
    });
...
describe('k6 Exporter', () => {
    ...
    it('generates valid k6 script for steady traffic', () => {
        ...
        const output = exporter.generate(config, 'https://example.com', 100);
    ...
    it('includes thresholds when results provided', () => {
        ...
        const output = exporter.generate(config, 'https://example.com', 100, results);
    ...
    it('validates high RPS', () => {
        const config = makeConfig({
            producer: {
                traffic: { pattern: 'steady', params: { rps: 200000 } },
            },
        });
        const result = exporter.validate(config);
...
describe('JMeter Exporter', () => {
    ...
    it('generates valid JMX XML for steady traffic', () => {
        ...
        const output = exporter.generate(config, 'https://example.com/api', 100);
    ...
    it('parses URL components correctly', () => {
        const config = makeConfig();
        const output = exporter.generate(config, 'https://api.example.com:8443/v1/test', 100);
...
describe('Cross-exporter pattern coverage', () => {
    const frameworks = ['k6', 'gatling', 'locust', 'jmeter', 'artillery'];
    const patterns = [
        {
            name: 'steady',
            config: { producer: { traffic: { pattern: 'steady', params: { rps: 500 } } } },
        },
        {
            name: 'gradual',
            config: { producer: { traffic: { pattern: 'gradual', params: { start_rps: 50, end_rps: 800 } } } },
        },
        {
            name: 'spike',
            config: { producer: { traffic: { pattern: 'spike', params: { base_rps: 200, spike_rps: 2000, spike_start: 120, spike_duration: 60 } } } },
        },
        {
            name: 'wave',
            config: { producer: { traffic: { pattern: 'wave', params: { base_rps: 300, amplitude: 200, period: 120 } } } },
        },
        {
            name: 'step',
            config: { producer: { traffic: { pattern: 'step', params: { steps: [{ rps: 100, duration: 120 }, { rps: 300, duration: 120 }] } } } },
        },
        {
            name: 'custom',
            config: { producer: { traffic: { pattern: 'custom', params: { series: [{ t: 0, rps: 100 }, { t: 60, rps: 500 }, { t: 120, rps: 200 }] } } } },
        },
    ];
...
describe('Edge cases', () => {
    it('handles very short duration', () => {
        ...
        for (const framework of ['k6', 'gatling', 'locust', 'jmeter', 'artillery']) {
            const exporter = svc.getExporter(framework);
            const output = exporter.generate(config, 'https://example.com', 100);
...
//# sourceMappingURL=loadtest.test.js.map