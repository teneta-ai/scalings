...
describe('SimulationService — backpressure capacity degradation', () => {
    ...
    it('does not degrade capacity when queue is below threshold', async () => {
        ...
        for (const snap of result.snapshots) {
            assert.equal(snap.effective_capacity_rps, snap.capacity_rps, `t=${snap.time}: effective capacity should equal base when queue (${snap.queue_depth}) is below threshold`);
        }
    });
